...
class AliyunProvider {
  ...
  getLogProjectResource() {
    ...
    return {
      ...
      "Properties": {
        ...
        "description": "Log project for Function Compute service "+ service + ", generated by the Serverless framework"
      }
    };
  }

  getLogStoreResource(funcName) {
    const storeName = this.getLogStoreName(funcName);
    return {
      "Type": "ALIYUN::SLS::Store",
      "Properties": {
        "storeName": storeName,
        "description": "Log store for Function Compute function " + funcName + ", generated by the Serverless framework",
        "ttl": 7,  // days
        "shardCount": 2
      }
    };
  ...
  getExecRoleResource() {
    ...
    return {
      ...
      "Properties": {
        "RoleName": roleName,
        "Description": "Allow Function Compute Service " + service +
          " to access other services, generated by the Serverless framework",
  ...
  letRoleAccessLog(resource) {
    const policies = resource.Properties.Policies;
    const writePolicy = this.getLogWritePolicy();
    const foundPolicy = policies.find((item) => _.isEqual(writePolicy));
    if (!foundPolicy) {
  ...
  getLogWritePolicy() {
    ...
    return {
      "PolicyName": `fc-access-${project}`,
      "Description": "Allow Function Compute to access log project " + project,
      "PolicyDocument": {
        "Version": "1",
        "Statement": [{
          "Action": [
